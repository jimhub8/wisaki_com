...
if ( ! function_exists( 'woodmart_image_hotspot_shortcode' ) ) {
	function woodmart_image_hotspot_shortcode( $atts, $content ) {
		...
		?>
		<div class="woodmart-image-hotspot-wrapper<?php echo esc_attr( $classes ); ?>">
			<div class="woodmart-image-hotspot-hotspots">
				<?php 
					if ( function_exists( 'wpb_getImageBySize' ) && $img ) {
						echo wpb_getImageBySize( array( 'attach_id' => $img, 'thumb_size' => $img_size, 'class' => 'woodmart-image-hotspot-img' ) )['thumbnail'];
					}
	...
	function woodmart_hotspot_shortcode( $atts, $content ) {
		...
		if ( $hotspot_type == 'product' && $product ) {
			...
			$output = '<div class="hotspot-product hotspot-content' . esc_attr( $content_classes ) . '">';
				...
				$output .= '<div class="hotspot-content-text woodmart-more-desc reset-mb-10"><div class="woodmart-more-desc-inner">' . do_shortcode( $product->get_short_description() ) . '</div></div>';
...
if( ! function_exists( 'woodmart_get_hotspot_image' ) ) {
	function woodmart_get_hotspot_image() {
		...
		$response = array(
			'status' => 'success',
			'html' => '<img class="woodmart-hotspot-img" src="' . esc_url( $background_image[0] ) . '" width="' . esc_attr( $background_image[1] ) . '" height="' . esc_attr( $background_image[2] ) . '">',
		);
