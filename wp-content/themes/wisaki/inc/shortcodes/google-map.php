...
if( ! function_exists( 'woodmart_shortcode_google_map' ) ) {
	function woodmart_shortcode_google_map( $atts, $content ) {
		...
		$map_args = array(
			...
			'marker_text'        => '<h3 style="min-width:300px; text-align:center; margin:15px;">' . $title . '</h3>' . $marker_text,
			'selector'           => 'woodmart-map-id-' . $uniqid,
		);
		...
		?>
			<div class="google-map-container <?php echo esc_attr( $el_class );?>" style="height:<?php echo esc_attr( $height ); ?>px;" data-map-args='<?php echo wp_json_encode( $map_args ); ?>'>
				...
				<?php if ( 'button' === $init_type ): ?>
					<div class="woodmart-init-map-wrap">
						<a href="#" class="btn btn-color-white woodmart-init-map">
				...
				<div class="woodmart-google-map-wrapper">
					<div id="woodmart-map-id-<?php echo esc_attr( $uniqid ); ?>" class="woodmart-google-map without-content"></div>
				</div>
