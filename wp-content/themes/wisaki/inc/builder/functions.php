...
if ( ! function_exists( 'xts_get_header_css' ) ) {
	/**
	 * Get header css.
	 *
	 * @since 1.0.0
	 * @return string
	 */
	function xts_get_header_css() {
		return WOODMART_HB_Frontend::get_instance()->get_header_css( whb_get_settings() );
	}
}

if ( ! function_exists( 'xts_get_header_elements_css' ) ) {
	/**
	 ...
	 * @return string
	 */
	function xts_get_header_elements_css() {
		$id   = whb_get_header()->get_id();
		$data = get_option( 'whb_' . $id );
		
		if ( isset( $data['elements_css'] ) ) {
			return $data['elements_css'];
		} else {
			whb_get_header()->set_elements_css( $data['structure'] );
			return whb_get_header()->get_elements_css();
		}
	}
}
