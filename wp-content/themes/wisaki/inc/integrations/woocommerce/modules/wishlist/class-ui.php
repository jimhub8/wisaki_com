...
class Ui extends Singleton {
	...
	public function wishlist_page_content( $wishlist = false ) {
		...
		if ( ! $this->is_editable() ) {
			$wrapper_classes .= ' woodmart-wishlist-preview';
		}
		
		ob_start();

		?>
			<div class="woodmart-wishlist-content<?php echo esc_attr( $wrapper_classes ); ?>">
				<?php if ( count( $products ) > 0 ) : ?>
					<div class="woodmart-wishlist-heading-wrapper">
						<h4 class="woodmart-wishlist-title">
	...
	public function remove_btn() {
		?>
			<div class="woodmart-button-remove-wrap">
				<a href="#" class="woodmart-wishlist-remove woodmart-button-remove" data-key="<?php echo esc_attr( wp_create_nonce( 'woodmart-wishlist-remove' ) ); ?>" data-product-id="<?php echo esc_attr( get_the_ID() ); ?>">
	...
	public function add_to_wishlist_btn( $classes = '' ) {
		?>
			<div class="woodmart-wishlist-btn <?php echo esc_attr( $classes ); ?>">
				<a href="<?php echo esc_url( woodmart_get_whishlist_page_url() ); ?>" data-key="<?php echo esc_attr( wp_create_nonce( 'woodmart-wishlist-add' ) ); ?>" data-product-id="<?php echo esc_attr( get_the_ID() ); ?>" data-added-text="<?php esc_html_e( 'Browse Wishlist', 'woodmart' ); ?>"><?php esc_html_e( 'Add to wishlist', 'woodmart' ); ?></a>
			</div>
		<?php
	}
	...
	public function account_navigation_classes( $classes, $endpoint ) {
		...
		if ( 'wishlist' === $endpoint && get_the_ID() == woodmart_get_opt( 'wishlist_page' ) ) {
			$classes[] = 'is-active';
		} elseif ( get_the_ID() == woodmart_get_opt( 'wishlist_page' ) ) {
