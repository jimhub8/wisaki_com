...
if ( ! class_exists( 'YITH_WCFM_Auctions' ) ) {
	...
	class YITH_WCFM_Auctions {
		...
		public function __construct() {
			...
			$YITH_Auction_Admin = YITH_Auctions()->admin;

			if ( empty( $YITH_Auction_Admin ) ) {
				$YITH_Auction_Admin_Class = $this->include_admin_class();
                $YITH_Auction_Admin       = new $YITH_Auction_Admin_Class();
			}
            /* === Save Product === */

            add_action( 'yith_wcfm_product_save',array( $YITH_Auction_Admin, 'save_auction_data' ) );
