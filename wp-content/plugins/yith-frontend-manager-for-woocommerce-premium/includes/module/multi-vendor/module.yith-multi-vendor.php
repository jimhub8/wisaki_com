...
if ( ! class_exists( 'YITH_Frontend_Manager_For_Vendor' ) ) {
	...
	class YITH_Frontend_Manager_For_Vendor {
		...
		public function render_product_columns( $columns ) {
			if ( $this->vendor->is_valid() && $this->vendor->has_limited_access() && 'no' == $this->vendor->featured_products_management() ) {
		...
		/**
		 * Skip show taxnomy boc for taxonomy without term vendor to product if current user is a vendor
		 *
		 * @param $skip bool
		...
		public function check_for_vendor_chat() {
			return YITH_Live_Chat()->multivendor_check();
		}
