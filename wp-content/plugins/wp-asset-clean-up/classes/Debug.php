...
class Debug
{
	...
	public function __construct()
	{
        ...
        if (isset($_GET['wpacu_get_cache_dir_size']) && is_admin()) { // Only print within the Dashboard
    ...
    {
	    $assetCleanUpCacheDir  = WP_CONTENT_DIR . OptimizeCommon::getRelPathPluginCacheDir();

	    echo '<h3>'.WPACU_PLUGIN_TITLE.': Caching Directory Stats</h3>';

	    if (is_dir($assetCleanUpCacheDir)) {
		    if (! is_writable($assetCleanUpCacheDir)) {
			    echo '<span style="color: red;">The directory is <strong>not writable</strong>.</span>' . '<br /><br />';
		    } else {
			    echo '<span style="color: green;">The directory is <strong>writable</strong>.</span>' . '<br /><br />';
		    }
		    ...
		    foreach (
			    ...
			    echo $item.' (<em>'.Misc::formatBytes($item->getSize()).'</em>)<br />';
			    if ( $item->isFile() ) {
