...
SQL;
	...
	/**
	 * Source: https://stackoverflow.com/questions/9150726/unset-inside-array-walk-recursive-not-working
	 *
	 * @param $array
	 */
	...
	public static function scriptExecTimer($name, $action = 'start')
	{
		...
		if ($action === 'start') {
			wp_cache_add($wpacuStartTimeName, (microtime(true) * 1000), 'wpacu_exec_time');
		}
		...
		if ($action === 'end' && ($startTime = wp_cache_get($wpacuStartTimeName, 'wpacu_exec_time'))) {
			...
			wp_cache_add($wpacuExecTimeName, $scriptExecTime, 'wpacu_exec_time');
