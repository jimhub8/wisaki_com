	...
	public static function alterHtmlSource($htmlSource)
	{
		...
		if (array_key_exists('wpacu_debug', $_GET)) {
			...
			foreach ( $timingKeys as $timingKey ) {
				...
				$wpacuTimingFormatMs = number_format($wpacuExecTiming, 2);
				$wpacuTimingFormatS = number_format(($wpacuExecTiming / 1000), 3);
				...
				$htmlSource = str_replace(
					...
					array(
						str_replace('.00', '', $wpacuTimingFormatMs) . 'ms',
						str_replace('.00', '', $wpacuTimingFormatS) . 's',
					), // clean it up
					$htmlSource );
			}
		}
