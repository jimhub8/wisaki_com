	...
	class YITH_Advanced_Refund_System_My_Account {
		...
		public function endpoint_title( $title ) {
			...
			if ( $is_my_refund_requests && ! is_admin() && is_main_query() && in_the_loop() && is_account_page() ) {
				$title = __( 'My Refund Requests', 'yith-advanced-refund-system-for-woocommerce' );
				...
				if ( $request->exists() ) {
					$title = sprintf( __( 'Request #%s', 'yith-advanced-refund-system-for-woocommerce' ), $request_id );
					remove_filter( 'the_title', array( $this, 'endpoint_title' ) );
				}
			}
		...
		public function add_my_refund_requests_link( $items ) {
			$items['refund-requests'] = __( 'My Refund Requests', 'yith-advanced-refund-system-for-woocommerce' );
		...
		public function my_refund_requests_content() {
			...
			wc_get_template( 'my-refund-requests.php',
				array( 'request_ids' => $request_ids ),
				'',
				YITH_WCARS_WC_TEMPLATE_PATH . 'myaccount/' );
		}
		...
		public function view_request_content() {
			...
			if ( $request->exists() ) {
				...
				if ( ! current_user_can( 'view_order', $request->order_id ) ) {
					echo '<div class="woocommerce-error">' . __( 'Invalid refund request.', 'yith-advanced-refund-system-for-woocommerce' ) . ' <a href="' . wc_get_page_permalink( 'myaccount' ) . '" class="wc-forward">' . __( 'My account', 'yith-advanced-refund-system-for-woocommerce' ) . '</a>' . '</div>';
					return;
				}

				wc_get_template( 'view-request.php',
		...
		public function view_request_shortcode( $atts ) {
			...
			if ( ! empty( $fields['id'] ) ) {
                ...
                <table class="ywcars_refund_info">
                    ...
                    <tr>
                        <td>
                            <span><?php echo __( 'Requested:', 'yith-advanced-refund-system-for-woocommerce' ) . ' '; ?></span>
		                    <?php if ( $is_whole_order ) : ?>
                                <span class="ywcars_bold"><?php _e( 'Whole order', 'yith-advanced-refund-system-for-woocommerce' ); ?></span>
		                    <?php else : ?>
			                    <?php
                        ...
                        <td class="ywcars_refund_info_minor_cell">
                            <a class="ywcars_reduced_text_size" href="<?php echo $order_link; ?>">
                                <span style="font-size: 8pt;"><?php echo __( 'View Order', 'yith-advanced-refund-system-for-woocommerce' ) . ' >'; ?></span>
                            </a>
                        </td>
                    </tr>
                    <tr>
                        <td colspan="2">
                            <span class="ywcars_current_status <?php echo $request->status; ?>">
                                <span><?php _e( 'Current status: ', 'yith-advanced-refund-system-for-woocommerce' ); ?></span>
                                <span class="ywcars_bold"><?php echo 'ywcars-new' == $request->status ? __( 'Submitted', 'yith-advanced-refund-system-for-woocommerce' ) : ywcars_get_request_status_by_key( $request->status ); ?></span>
                            </span>
                            <div class="ywcars_reduced_text_size"><?php _e( 'Request date:', 'yith-advanced-refund-system-for-woocommerce' ); ?></div>
                    ...
                    <?php if ( 'ywcars-coupon' == $request->status && $request->coupon_id ) : ?>
	                    ...
	                    $coupon_code = ! empty( $post->post_title ) ? $post->post_title : __( 'The coupon code does no longer exists', 'yith-advanced-refund-system-for-woocommerce' );
	                    $coupon_amount = get_post_meta( $request->coupon_id, 'coupon_amount', true );
	                    ?>
                        <tr>
                            <td>
                                <div class="ywcars_reduced_text_size"><?php _e( 'Coupon code:', 'yith-advanced-refund-system-for-woocommerce' ) ?></div>
                                <div class="ywcars_bold"><?php echo $coupon_code; ?></div>
                            </td>
                            <td class="ywcars_refund_info_minor_cell">
                                <div class="ywcars_reduced_text_size"><?php echo _x( 'value', 'Value of the coupon', 'yith-advanced-refund-system-for-woocommerce' ) ?></div>
                                <div class="ywcars_bold" style="font-size: 14pt;"><?php echo ! empty( $coupon_amount ) ? wc_price( $coupon_amount ) : _x( 'N/A', 'Coupon amount value', 'yith-advanced-refund-system-for-woocommerce' ); ?></div>
                            </td>
                        </tr>
                    <?php endif; ?>
                    </tbody>
				...
				<?php if ( ! $request->is_closed ) : ?>
                    <div class="ywcars_refund_info_add_mesage">
                        <form id="ywcars_form_my_account_new_message" method="post" enctype="multipart/form-data">
                            <div class="ywcars_refund_info_block_title"><?php _e( 'Add a message', 'yith-advanced-refund-system-for-woocommerce' ); ?></div>
                            <textarea id="ywcars_new_message" name="ywcars_new_message"
                                      title="<?php _e( 'Write a message you want to sent to the Shop Manager.',
                                          'yith-advanced-refund-system-for-woocommerce' ); ?>"
                                      rows="5"></textarea>
                            <div class="ywcars_block">
                                <input type="submit" class="button button-primary"
                                       id="ywcars_send_message" style="float: right;"
                                       value="<?php _e( 'Submit', 'yith-advanced-refund-system-for-woocommerce' ); ?>">
				...
				<?php else : ?>
                    <div>
                        <span><?php _e( 'This request is closed. You cannot send further messages.', 'yith-advanced-refund-system-for-woocommerce' ); ?></span>
                    </div>
				<?php endif; ?>
                ...
                <div class="ywcars_refund_info_messages_history">
                    ...
                    <div id="ywcars_message_history" class="ywcars_refund_info_block_title">
                        <span><?php _e( 'Message history', 'yith-advanced-refund-system-for-woocommerce' ); ?></span>
                        <span class="ywcars_update_messages" title="<?php _e( 'Reload', 'yith-advanced-refund-system-for-woocommerce' ); ?>">&#x21bb;</span>
                    </div>
