...
if ( ! class_exists( 'YITH_Advanced_Refund_System_Request_Manager' ) ) {
    ...
    class YITH_Advanced_Refund_System_Request_Manager {
	    ...
	    public function process_automatic_request( $request_id ) {
		    ...
		    $refund_reason          = __( 'Automatic refund', 'yith-advanced-refund-system-for-woocommerce' );
		    ...
		    try {
			    ...
			    if ( ! $refund_amount || $max_refund < $refund_amount || 0 > $refund_amount ) {
				    throw new exception( __( 'Invalid refund amount', 'yith-advanced-refund-system-for-woocommerce' ) );
			    }
				    ...
				    if ( $api_refund ) {
					    ...
					    if ( isset( $payment_gateways[ $order->payment_method ] ) && $payment_gateways[ $order->payment_method ]->supports( 'refunds' ) ) {
						    ...
						    if ( is_wp_error( $result ) ) {
							    ...
							    throw new Exception( __( 'Refund failed', 'yith-advanced-refund-system-for-woocommerce' ) );
						    }
					    }
				    }
				    ...
				    foreach ( $line_item_qtys as $item_id => $qty ) {
					    if ( $restock_refunded_items && $qty && isset( $order_items[ $item_id ] ) ) {
						    ...
						    if ( $_product && $_product->exists() && $_product->managing_stock() ) {
							    ...
							    $order->add_order_note( sprintf( __( 'Item #%s stock increased from %s to %s.', 'yith-advanced-refund-system-for-woocommerce' ), $order_item['product_id'], $old_stock, $new_quantity ) );
