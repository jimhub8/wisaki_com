...
<?php if ( ! $request_ids ) : ?>
    <p><?php echo apply_filters( 'ywcars_no_requests_found', __( 'There are currently no refund requests, if you would like to request a refund, then please navigate to orders page and select which order or product you want to request a refund for.', 'yith-advanced-refund-system-for-woocommerce' ) ); ?></p>
<?php else : ?>
    <table class="shop_table shop_table_responsive my_account_orders">
		...
		foreach ( $request_ids as $request_id ) {
			...
			if ( $request->whole_order ) {
				$product_link = '<b>' . __( 'Whole Order', 'yith-advanced-refund-system-for-woocommerce' ) . '</b>';
			} else {
			...
			$button = '<a class="button" href="' . esc_url( $request->get_view_request_url() ) . '">'
			          . __( 'View', 'yith-advanced-refund-system-for-woocommerce' ) . '</a>';
			?>
            <tr>
                ...
                <td><?php echo 'ywcars-new' == $request->status ? __( 'Submitted', 'yith-advanced-refund-system-for-woocommerce' ) : ywcars_get_request_status_by_key( $request->status ) ?></td>
