...
<div class="ywcars_items_info">
	<?php
	$href       = admin_url( 'post.php?post=' . yit_get_order_id( $order ) . '&action=edit' );
	$title      = sprintf( __( 'Go to order #%d', 'yith-advanced-refund-system-for-woocommerce' ), yit_get_order_id( $order ) );
	$order_link = '<a href="' . $href . '" title="' . $title . '">#' . yit_get_order_id( $order ) . '</a>';
    ...
    <h2 style="display: inline-block; vertical-align: super;"><?php printf( _x( 'Request %s for order %s', 'Request [request ID] for order [order ID]', 'yith-advanced-refund-system-for-woocommerce' ), '#' . $request->ID, $order_link );
		?></h2>
    <div class="ywcars_before_items_table">
		...
		if ( $request->whole_order ) {
			$info = sprintf( __( '%s made a request for the whole order', 'yith-advanced-refund-system-for-woocommerce' ), $customer );
		} else {
			$span_product = $product ? '<span style="text-decoration: underline;">' . $product->get_title() . '</span>': '';
			$span_qty     = '<span title="' . __( 'Quantity requested', 'yith-advanced-refund-system-for-woocommerce' ) . '">' . $request->qty . '</span>';
			$info         = sprintf( __( '%s made a request for %s (%s)', 'yith-advanced-refund-system-for-woocommerce' ), $customer, $span_product, $span_qty );
		}
		?>
    ...
    <div style="overflow: auto;">
        <table class="ywcars_items_table">
            <thead>
            <tr>
                <th><?php _e( 'Item', 'yith-advanced-refund-system-for-woocommerce' ); ?></th>
                <th><?php _e( 'Item value', 'yith-advanced-refund-system-for-woocommerce' ); ?></th>
				<?php if ( $tax_enabled ) : ?>
                    <th><?php _e( 'Tax per item', 'yith-advanced-refund-system-for-woocommerce' ); ?></th>
				<?php endif; ?>
                <th><?php _e( 'Qty', 'yith-advanced-refund-system-for-woocommerce' ); ?></th>
				<?php if ( $tax_enabled ) : ?>
                    <th class="ywcars_items_table_totals"><?php _e( 'Item total', 'yith-advanced-refund-system-for-woocommerce' ); ?></th>
                    <th class="ywcars_items_table_totals"><?php _e( 'Tax total', 'yith-advanced-refund-system-for-woocommerce' ); ?></th>
				<?php endif; ?>
                <th class="ywcars_items_table_totals"><?php _e( 'Order Total', 'yith-advanced-refund-system-for-woocommerce' ); ?></th>
                <th class="ywcars_items_table_totals ywcars_items_table_refund"><?php _e( 'Qty to refund', 'yith-advanced-refund-system-for-woocommerce' ); ?></th>
                <th class="ywcars_items_table_totals ywcars_items_table_refund"><?php _e( 'Total to be refunded',
			...
			foreach ( $items as $item_id => $item ) {
				...
				if ( 'line_item' == $item_type ) :
					...
					if ( ! $product || ! $product->exists() ) {
						?>
                        <tr>
                            <td><?php echo __( 'The requested product does not exist.', 'yith-advanced-refund-system-for-woocommerce' ); ?></td>
                        </tr>
						<?php
						continue;
					}
				...
				<?php else : ?>
                    <tr class="ywcars_non_line_item">
						...
						<?php if ( 'shipping' == $item_type ) : ?>
                            ...
                            <td><?php echo $item_name ? $item_name : __( 'Shipping', 'yith-advanced-refund-system-for-woocommerce' ); ?></td>
                            <td></td><td></td>
							<?php if ( $tax_enabled ) : ?>
                                <td></td>
						...
						<?php if ( 'fee' == $item_type ) : ?>
                            ...
                            <td><?php echo $item_name ? $item_name : __( 'Fee', 'yith-advanced-refund-system-for-woocommerce' ); ?></td>
                            <td></td><td></td>
							<?php if ( $tax_enabled ) : ?>
                                <td></td>
            ...
            <tr>
				...
				<?php if ( $tax_enabled ) : ?>
                    <td></td>
				<?php endif; ?>
                <td class="ywcars_items_table_totals"><?php _e( 'Totals', 'yith-advanced-refund-system-for-woocommerce' ); ?></td>
	...
	<?php if ( YITH_Advanced_Refund_System_Request_Manager::order_has_wc_refunds( $order ) ) : ?>
        <div>
            <h4><?php _e( 'Refunds managed by WooCommerce has been done on the order.', 'yith-advanced-refund-system-for-woocommerce' ); ?></h4>
        </div>
	<?php endif; ?>

	<?php if ( $request->is_closed ) : ?>
        <div>
            <h4><?php _e( 'This request is currently closed.', 'yith-advanced-refund-system-for-woocommerce' ); ?></h4>
        </div>
	<?php endif; ?>
        ...
        <div class="ywcars_after_items_table">
            <div>
                ...
                <span>
                    ...
                    <label for="ywcars_restock_items"><?php _e( 'Restock selected items?', 'yith-advanced-refund-system-for-woocommerce' ) ?></label>
                </span>
                </p>
                <p class="ywcars_after_items_table_block">
                <span>
                    <span><?php _e( 'Use a custom refund amount', 'yith-advanced-refund-system-for-woocommerce' ) ?></span>
            ...
            <div>
				...
				$gateway_name             = false !== $payment_gateway ? ( ! empty( $payment_gateway->method_title ) ? $payment_gateway->method_title : $payment_gateway->get_title() ) : __( 'Payment Gateway', 'yith-advanced-refund-system-for-woocommerce' );
                ...
                <button id="ywcars_api_refund_button"
					...
					printf( _x( 'Refund %s via %s', 'Refund [amount] via [gateway name]', 'yith-advanced-refund-system-for-woocommerce' ), $refund_amount, $gateway_name );
					?></button>
                <button id="ywcars_manual_refund_button"
					...
					printf( _x( 'Refund %s manually', 'Refund $amount manually', 'yith-advanced-refund-system-for-woocommerce' ), $refund_amount );
					?></button>
                ...
                <button class="button button-secondary" id="ywcars_reject_request_button">
                    <span><?php _e( 'Reject request', 'yith-advanced-refund-system-for-woocommerce' ); ?></span>
                </button>
                <button class="button button-secondary" id="ywcars_processing_request_button">
                    <span><?php _e( 'Set to processing', 'yith-advanced-refund-system-for-woocommerce' ); ?></span>
                </button>
                <button class="button button-secondary" id="ywcars_on_hold_request_button">
                    <span><?php _e( 'Set to on hold', 'yith-advanced-refund-system-for-woocommerce' ); ?></span>
                </button>
				<?php do_action( 'ywcars_after_action_buttons', $request, $refund_amount, $tax_enabled ); ?>
            </div>
        </div>
	<?php endif; ?>
	<?php if ( $finished_request && ! $request->is_closed ) : ?>
        <div>
            <button class="button button-secondary tips" id="ywcars_close_request_button"
                    data-tip="<?php _e( 'Close the request by stopping the messages system. This action prevents you from changing the request status.', 'yith-advanced-refund-system-for-woocommerce' ); ?>">
                <span><?php _e( 'Close this request', 'yith-advanced-refund-system-for-woocommerce' ); ?></span>
            </button>
        </div>
	<?php endif; ?>
